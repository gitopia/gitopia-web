...
function install {
	...
	*) fail "unknown os: $(uname -s)";;
	...
	case "${OS}_${ARCH}" in
	"darwin_amd64")
		URL="$OBJECTS_URL/releases/$USER/git-remote-gitopia/v0.3.0/git-remote-gitopia_0.3.0_darwin_amd64.tar.gz"
		FTYPE=".tar.gz"
		;;
	"darwin_arm64")
		URL="$OBJECTS_URL/releases/$USER/git-remote-gitopia/v0.3.0/git-remote-gitopia_0.3.0_darwin_arm64.tar.gz"
		FTYPE=".tar.gz"
		;;
	"linux_amd64")
		URL="$OBJECTS_URL/releases/$USER/git-remote-gitopia/v0.3.0/git-remote-gitopia_0.3.0_linux_amd64.tar.gz"
		FTYPE=".tar.gz"
		;;
	*) fail "No asset for platform ${OS}-${ARCH}";;
	...
	if [[ $FTYPE = ".gz" ]]; then
		...
		GZURL="$OBJECTS_URL/releases/$USER/git-remote-gitopia/$RELEASE/$NAME"
		#gz download!
