      ...
      for (const bounty of issue.bounties) {
        for (const amount of bounty.bounty.amount) {
          ...
          if (existingAmount) {
            ...
            console.log(`Adding ${parsedAmount} to existing ${denomName} for issue ${issueId}. Total: ${existingAmount.amount}`);
          } else {
            groupedBounties[issueId].cumulativeAmount.push({
              amount: parsedAmount,
              denomName: denomName,
            });
            console.log(`Creating new entry for ${denomName} with amount ${parsedAmount} for issue ${issueId}`);
          }
        }
      }
    }
